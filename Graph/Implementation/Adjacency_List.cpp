...
void Graph::display(){
    ...
    for(int i=0;i<unweighted_Graph.size();i++){
        cout<<i<<" -> ";
        for(int j=0;j<unweighted_Graph[i].size();j++){ // Printing the Neighbours of Vertex i
            int ele = unweighted_Graph[i].front();
            if(j<unweighted_Graph[i].size()-1)cout<<ele<<",";
            else cout<<ele;
            unweighted_Graph[i].pop_front();
            unweighted_Graph[i].push_back(ele);
        }
        cout<<endl;
    }
}
...
int main(){
    ...
    cout<<"Program is based on Unweighted Graph implementation using Adjacency List approach\n";
    Graph G;
    G.createGraph();
    G.display();
}